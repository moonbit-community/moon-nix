...
def evaluate_plan(expression):
    with tempfile.NamedTemporaryFile(mode="w", suffix=".nix") as generated:
        generated.write(expression)
        generated.flush()
        environment = os.environ.copy()
        environment["HOME"] = str(Path(generated.name).parent)
...
assert original.stdout.startswith("# Generated by moon2nix. Do not edit.\n{")
plan = evaluate_plan(original.stdout)
expected = evaluate_plan((examples / "wasm-plan.nix").read_text())
...
with tempfile.TemporaryDirectory() as temporary:
    ...
    assert evaluate_plan(relocated.stdout) == plan
